#define UNIFEX_EPOLL_TEST_WRITE_THREAD 1
#define UNIFEX_EPOLL_TEST_WHEN_ALL_WRITES 0

#if UNIFEX_EPOLL_TEST_WRITE_THREAD
/*
...
int main() {
  ...
  std::thread w{[&, wPipe = std::move(wPipe)] () mutable { 
      try {
  ...
  auto pipe_bench = [&, rPipe = std::move(rPipe)](int seconds, auto& stopSource) mutable {
    return unstoppable(
      with_query_value(
        discard(
          when_all(
...
#elif UNIFEX_EPOLL_TEST_WHEN_ALL_WRITES
          ...
          sequence(
            ...
            with_query_value(
              repeat(
                discard(
                  async_write_some(wPipe, databuffer))),
              get_stop_token, stopRead.get_token()),
            lazy([&]{
              printf("writes completed!\n");
            })),
          sequence(
...
#else // !UNIFEX_EPOLL_TEST_WHEN_ALL_WRITES && UNIFEX_EPOLL_TEST_WRITE_THREAD
#include <cstdio>
int main() {
  printf("epoll tests disabled\n");
}
#endif // !UNIFEX_EPOLL_TEST_WHEN_ALL_WRITES && UNIFEX_EPOLL_TEST_WRITE_THREAD
